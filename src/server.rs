...
pub fn run_server<SE, CE>(server_handle:ServerHandle<SE, CE>, bind_address: SocketAddr) -> PsykResult<ServerPoisonPill> where SE : Deserialize + Send + Clone + 'static, CE : Deserialize + Send + Clone + 'static { // spawns a server and returns a poison pill handle ... that can be used to terminate the server
...
pub fn create_server<SE, CE>(server_handle:ServerHandle<SE, CE>, bind_address: SocketAddr, poison_receiver: oneshot::Receiver<u32>) where SE : Deserialize + 'static + Clone, CE : Deserialize + 'static + Clone {
    ...
    let srv = socket.incoming().for_each(move |(socket, addr)| {
        ...
        let receiving = stream.for_each(move |m| {
        ...
        let done = receiving.then(move |res| {
            println!("uhhh, what's res fucko -> {:?}", res);

            other_handle.sender.send(ServerEvent::ClientDisconnected { address : addr })
            // Ok(())
            // Err(())
        }).map_err(|_| ()).map(|_| ());
